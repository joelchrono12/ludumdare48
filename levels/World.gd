...
func _ready() -> void:
	...
	tween.interpolate_property(cam,"position",cam.position,player.position,1,Tween.EASE_IN,Tween.EASE_OUT,0)
	tween.interpolate_property(cam,"zoom",cam.zoom,Vector2(1,1),1,Tween.EASE_IN,Tween.EASE_OUT,0)
	lava_tween.interpolate_property(lava,"position",lava.position,Vector2(0, chosen_position),15,Tween.TRANS_LINEAR,Tween.EASE_OUT,0)
	start_timer.start()
...
func _on_CamTweener_tween_all_completed() -> void:
	...
	player.cam_shake.play("cam_shake (copy)")
	lava_tween.start()
